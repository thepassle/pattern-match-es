...
async function startTestRunner(options = {}) {
    ...
    try {
        ...
        if (groupConfigs.find(g => g.name === 'default')) {
            throw new Error('Cannot create a group named "default". This named is reserved by the test runner.');
        }
        ...
        if (config.nodeResolve) {
            ...
            config.plugins.unshift(nodeResolvePlugin_1.nodeResolvePlugin(rootDir, config.preserveSymlinks, userOptions));
        }
        if (config.esbuildTarget) {
            config.plugins.unshift(esbuildPlugin_1.esbuildPlugin(config.esbuildTarget));
        }
        config.plugins.unshift(plugins_1.setViewportPlugin(), plugins_1.emulateMediaPlugin(), plugins_1.setUserAgentPlugin());
